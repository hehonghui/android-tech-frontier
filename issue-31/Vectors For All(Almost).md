...
但如果我们不需要全部的分辨率而更关注APK增加的尺寸。我们可以通过build flavor的`generatedDensities`属性来控制需要生成的分辨率。
...
如果我们现在build一下（记得先clean来清除之前生成的资源），就会发现只生成了我们制定的分辨率的资源：
...
这和我手动添加缺少的transformations之前导入的SVG渲染出的一样。我本应该提到之前lint有一个警告指出 元素，但是这背离了*VectorDrawable*是一个Android的特定格式，所以没有完整的支持是让人困惑的。
...
to PNG.  
现在我们开始理解为什么transformations不被导入工具支持了，因为transformations在*VectorDrawable*被转换成光栅化图片以向后兼容时是不支持的。这应该是一个主要的遗漏：完全有效的*VectorDrawable*资源在Lollipop以及之后的系统上是可以完美渲染的，但是转换成PNG图片就不正确了。
...
总结下：如果你使用这些新工具从material icons library中导入资源，那么很完美。然而，声明导入工具可以导入SVG文件则是一种误导，因为它支持非常有限的子集，并且无法正确导入大部分实际真正使用的SVG文件。此外，缺少对*VectorDrawable*转换为光栅化视图的完整支持让人感觉这项功能没有真正完成，没有准备好投入使用。
...
使用导入工具配合手动调整将官方的SVG logo转成*VectorDrawable*并没有比我纯手动调整需要更多工作。尽管我仍然需要自己调整SVG pathData中的所有transformations的坐标，来实现transformations。
