...
* 代码混淆：使用方法分发，就如同自己写的代码一样

当然所有这些很棒的特点都需要付出一个代价，那就是缺乏灵活性。比如说：Dagger2由于没用反射所以没有动态机制。
...
* @Inject: 通常在外面需要依赖的地方注解。换句话说，你用它告诉Dagger这个类或者变量要加到依赖注入中。这样，Dagger就会构造一个这个类的实例并满足他们的依赖。
* @Module: Modules是提供依赖方法的类。我们定义一个类，用@Module注解，这样Dagger在构造类的实例的时候，就知道从哪里去找到需要的依赖。modules的一个重要特征是他们被设计为分开的并组成在一起（比如说，在我们的app中可以有多个组成在一起的modules）。
