...
你可能听过这么一句话 “最好的代码就是没有代码。” 然而我们并不建议不写任何代码，你编写的代码应该为你的应用程序增加了独特的价值而不是一些复制的样板代码。通过兼顾一些小模块，Android支持库是帮助你完成这一点的最好的资源之一。
...
## Support V4
Support V4库作为许多Android支持库的基础，也包含了许多兼容类使向后兼容变得更加容易。
...
此外，我们正在做一些[Palette](https://developer.android.com/reference/android/support/v7/graphics/Palette.html?utm_campaign=ASL221-415&utm_source=dac&utm_medium=blog)的内部构件，通过[ColorUtils](http://developer.android.com/reference/android/support/v4/graphics/ColorUtils.html?utm_campaign=ASL221-415&utm_source=dac&utm_medium=blog)类可供任何人使用，为你提供预先构建的工具以在多种颜色下更好的工作。ColorUtils可以很容易地计算出颜色之间的对比度，确定最小的透明度值去维持一个最小的对比度，或者转换颜色为其对应的HSL值（译者注：Hue[hju]色调，Saturation['sætʃə'reʃən]饱和度，Luminance['lumɪnəns]亮度）。
...
在使用AppCompat时自动着色控件的能力是在你的应用程序中保持强大品牌和一致性体验中起到难以置信的帮助和能力。这应该在编写布局时自然而然做的-AppCompatButton代替Button、AppCompatTextView代替TextView、等等，以确保他们能互相支持着色。在此版本中，那些
色彩感知控件现在已经公开，让你保持着色的支持即使你需要继承支持的部件之一。
...
Lollipop增加了在一个view中通过view级别使用android:theme XML属性重写主题的能力-非常有用的特性，如在亮色activities上的黑色action bars。现在，AppCompat允许你为Toolbars使用android:theme（不赞成使用之前的app:theme）,更好地，带来为API 11+的所有views的android:theme支持。
...
如果你刚开始接触AppCompat，可以察觉出是多么容易上手，并为你所有的用户带来了一致性的设计：

    **Video**
<iframe width="420" height="315"
src="https://youtu.be/5Be2mJzP-Uw">
</iframe>
...
## Leanback
Leanback库作为Android电视应用程序的最佳实践的集合，我们曾忽略去不使一个更美好的10的经验作为发行版的一部分（译者注：这里好拗口）。你会注意到加载后立即更新Leanback例子新功能的引导步骤。
...
## SDK 现在可用了！
没有比这更好的时间来开始使用Android支持库。今天你就可以开始开发了，从Android SDK Manager下载Android支持库和Android支持资源吧。
