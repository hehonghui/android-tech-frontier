...
Using the http://thecatapi.com/ API we can retrieve several funny cat images
KittenApp
...
- Retrofit2 for the network access, REST API calls and deserializing data
- Glide for showing images
- RxJava to bind data
- RecyclerView CardView as UI
- Everything wrapped in a MVP pattern
...
There are many ways to retrieve data through the network, and different libraries to handle it. One of these libraries is the Retrofit2 library from Square.
...
To design it we need basically of two elements the main container and the item container.
...
In any case you can find the entire project on my Github Repository of the KShow project. 

The same project has been written in Java as well JShows so you can comparing them
